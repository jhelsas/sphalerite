...
int main(int argc, char **argv){
  ...
  bool is_cll = true;
  print_time_stats("SoA,inner,serial",is_cll,N,h,seed,runs,lsph,box,times);
  print_sph_particles_density("SoA,inner,serial",is_cll,N,h,seed,runs,lsph,box);
              ...
              void *swap_arr, linkedListBox *box, SPHparticle *lsph, double *times)
{
  ...
  err = compute_density_3d_cll(N,h,lsph,box);
  if(err)
